...
const VoucherScreen = ({route}) => {
 ...
 const handleApplyVoucher =async (item) => {
    ...
    const getDefaultVouchers = async () => {
      ...
      try {
        ...
        if (response.status) {
          ...
          setVouchers([]);
          ToastAndroid.show('Không thể lấy danh sách voucher!', ToastAndroid.SHORT);
        }
      } catch (error) {
        setVouchers([]);
  ...
  const searchVouchers = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await AxiosInstance().get(`/vouchers/search?query=${searchQuery}`);
  ...
  return (
    <View style={[appst.container, style.container]}>
      ...
      leftOnPress={() => navigation.goBack()}
        ...
        name={'Vouchers'}
      />
      <SearchSection searchQuery={searchQuery} setSearchQuery={setSearchQuery} onSearch={searchVouchers} />
      {isLoading ? (
        <Text style={style.loadingText}>Đang tải...</Text>
      ) : vouchers.length > 0 ? (
        <FlatList
          data={vouchers}
          renderItem={({item}) => <VoucherItem item={item} handleApplyVoucher={handleApplyVoucher}/>}
        ...
        <Text style={style.noVoucherText}>Hiện tại chưa có voucher nào khả dụng</Text>
      )}
    </View>
  );
};
  ...
  return (
    <View style={[styleItem.container]}>
      ...
      <View style={styleItem.bottomView}>
        ...
        <View style={styleItem.headerContainer}>
          <Text style={styleItem.endow}>End date: {formatDate(item.expiry_date)}</Text>
          <TouchableOpacity style={[styleItem.buttonClaimContainer]}
