...
const DepositWalletScreen = ({route}) => {
  ...
  const handleDeposit = async () => {
    if (!value) {
      Toast.show({
        text2: 'Tiền không được bỏ trống!',
        type: 'error',
        position: 'bottom',
      });
      return;
    ...
    if (!selectedMethod) {
      Toast.show({
        text1: 'Vui lòng chọn ít nhất 1 phương thức thanh toán!',
        type: 'error',
        position: 'bottom',
      });
      return;
    ...
    try {
      ...
      if (response.status) {
        ...
        Toast.show({
          text2: response.message || 'Không thể tạo giao dịch.',
          type: 'error',
        });
      }
    } catch (error) {
      Toast.show({text1: 'Có lỗi xảy ra', type: 'error', position: 'bottom'});
    }
  };
    ...
    <View style={styles.container}>
      ...
      {orderUrl ? (
        <WebView
          ...
          onError={err =>
            Toast.show({text2: 'Lỗi khi tải trang', type: 'error',
              position: 'bottom',})
          }
        />
      ) : (
        <View style={styles.viewBody}>
          <View style={styles.viewInput}>
            ...
            <Text style={styles.currentBalance}>
              Số dư ví hiện tại: {formatVND(balance)} ₫
            </Text>
          ...
          <View style={styles.bottomContainer}>
            <Text style={styles.text}>
              ...
              <Text style={styles.TermOfUse}>Chính sách bảo mật</Text> của Shoes
              Mate.
            </Text>
            <CustomedButton
              title={'Nạp tiền ngay'}
