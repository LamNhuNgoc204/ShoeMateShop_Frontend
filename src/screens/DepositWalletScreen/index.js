...
const DepositWalletScreen = ({ route }) => {
  ...
  const handleDeposit = async() => {
    if (!value) {
      Toast.show({text2 :t('depositwallet.toastemrty'), type: 'error',position: 'bottom'});
      return;
    }

    if (!selectedMethod) {
      Toast.show({text1: t('depositwallet.emrtypayment'), type: 'error',position: 'bottom'});
      return;
    }

    if (selectedMethod === 'bank') {
      Toast.show({
       text1:  t('depositwallet.bankmaintenance'),
       type: 'error',
       position: 'bottom',
    });
      return;
    ...
    try {
      ...
      if (response.status) {
       ...
       Toast.show({text2 : response.message || t('depositwallet.cannottrade'), type: 'error',position: 'bottom'});
      }
    } catch (error) {
